...
class Gpib(object):
    def __init__(self, addr, serialport):
        if type(serialport) == str:
            self.port = SerialMod(serialport, 115200, timeout=1)
        else:
            self.port = serialport
        if self.port.isOpen() == False:
            self.port.open()
        self.addr = addr
        self.port.write(('++read_tmo_ms %d\n' % 1000).encode())
    ...
    def rsp(self):
        ...
        # while True:
        #     print('trying to read spoll')
        #     msgin = self.port.readline()
        #     print('msgin from spoll', repr(msgin))
        #     if len(msgin) > 0:
        #         break
        #     time.sleep(0.1)
        try:
